...
if (isset($_GET["invoiceId"])) {
    ...
    $Ubirthdate = $diferencia->y;

    $image =        './../../../../assets/image/system/sport/' . $data["Simage"];
    $imageMoney =        './../../../../assets/image/icons/3D-Money.avif';
    ...
    $PUid          = $dataPerson['Uid'];
    $Pname          = $dataPerson['Uname'];
    $Plastname      = $dataPerson['Ulastname'];
...
$pdf->MultiCell(150, 5, mb_convert_encoding("Lunes a Viernes", 'ISO-8859-1', 'UTF-8'), 0, 'L', false);
$pdf->Ln(2);
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetTextColor(39, 39, 51);
...
$pdf->Line(18, 185, 120, 185);
if ($imageVoucher == '') {
    $pdf->Image("$imageMoney", 125, 125, 75, 80);
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->SetTextColor(39, 39, 51);
    $pdf->Cell(165, 10, utf8_decode("Pago en efectivo"), 0, 0, 'R');
} else {
    $pdf->Image("$imageVoucher", 125, 147, 70, 100);
    $pdf->Rect(125, 147, 70, 100);
}
...
if ($PUid != null) {
    ...
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->SetTextColor(39, 39, 51);
    $pdf->MultiCell(100, 5, mb_convert_encoding("CAJERO: $Pname $Plastname", 'ISO-8859-1', 'UTF-8'), 0, 'C', false);
...
$pdf->SetFont('Arial', 'B', 10);
$pdf->SetTextColor(39, 39, 51);
$pdf->Cell(100, 5, utf8_decode("*** VALOR DEL CURSO VACACIONAL ***"), 0, 0, 'C');
$pdf->Ln(7);
$pdf->SetFont('Arial', 'B', 20);
$pdf->SetTextColor(39, 39, 51);
$pdf->MultiCell(100, 7, mb_convert_encoding("$TotalWithDecimal", 'ISO-8859-1', 'UTF-8'), 0, 'C', false);
$pdf->Ln(2);

if ($PSid == 1) {
    $pdf->SetFont('Arial', 'B', 20);
    $pdf->SetTextColor(255, 0, 0);
    $pdf->MultiCell(100, 7, mb_convert_encoding("$PSname", 'ISO-8859-1', 'UTF-8'), 0, 'C', false);
} else if ($PSid == 2) {
    $pdf->SetFont('Arial', 'B', 20);
...
if ($PUid != null) {
    ...
    $pdf->SetFont('Arial', '', 9);
    $pdf->SetTextColor(39, 39, 51);
    $pdf->MultiCell(0, 0, mb_convert_encoding("$Pname $Plastname", 'ISO-8859-1', 'UTF-8'), 0, 'C', false);
...
header('Content-Disposition: inline; filename="./../../../../assets/image/report/logo-100.png"');
header('Content-Transfer-Encoding: binary');
header('Accept-Ranges: bytes');

// Enviar el contenido del PDF al navegador
readfile('./../../../../assets/image/report/logo-100.png');
